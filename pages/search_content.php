<div id="main_content_cover" class="dfs">
	<div id="main_content">
		<?php
		...
		<?php
			...
			while($chronicleRow = mysqli_fetch_array($retval))
			{
				...
				if(!$chronicleRow['is_private'] && !$chronicleRow['is_blacklisted'])
				{
					...
					echo "<tr class='data_row'><td><div class='channel_status_data'>{$openStatusStr}</div></td><td><div class='channel_name_data'>{$chronicleRow['channel_name']}</div></td><td><div class='channel_owner_data'>{$chronicleRow['channel_owner']}</div></td><td><div class='warning_list_data'>{$warningListStr}</div></td><td><div class='link_data'><a href='http://chronicler.seanmfrancis.net/chronicle.php?id={$chronicleRow['channel_id']}&page=1'>Link To Chronicle</a></div></td><td><div class='date_data'>{$date}</div></td></tr>";              
